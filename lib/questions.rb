...
def word_count_a_file(file_path)
  File.open("./#{file_path}", "r").read.split(" ").count
end
...
def count_words_of_each_length_in_a_file(file_path)
  result, array = Hash.new(0), File.open("./#{file_path}","r").read.gsub!(/[^a-z0-9\s]/i, '').split(" ")
	array.each { |element| result[element.length] +=1 }
	result
end
